...
describe('dashboardLayout', () => {
  it('keeps the overview dashboard on a full-width work surface', () => {
    ...
    expect(dashboardLayout.page).toContain('gap-4')
  })
  ...
  it('uses compact desktop grids so the first viewport shows the cockpit, not a card feed', () => {
    ...
    expect(dashboardLayout.header).toContain('gap-4')
    expect(dashboardLayout.header).toContain('rounded-xl')
    expect(dashboardLayout.headerActions).toContain('min-w-0')
    expect(dashboardLayout.headerActions).toContain('items-center')
    ...
    expect(dashboardLayout.panel).toContain('rounded-xl')
  })
})
