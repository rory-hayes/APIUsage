...
describe('admin runs page', () => {
  let tempDir: string

  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'uri-admin-runs-page-'))
  ...
  it('shows parser and reconciliation runs for the active workspace only', async () => {
    ...
    const page = await AdminRunsPage()
