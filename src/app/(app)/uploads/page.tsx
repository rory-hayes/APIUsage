...
import { addUploadTaskCommentAction, uploadFileAction } from './actions'
...
export default async function UploadsPage() {
  ...
  const uploadChecklist = getWorkspaceRecurringUploadChecklist(persistedUploads, workspace, requiredUploadCategories)
  const comments = commentsByCategory(await getUploadTaskCommentStore().listByWorkspace(workspace.id))
  const checklistPeriodLabel = uploadChecklist.find((item) => item.periodLabel)?.periodLabel ?? workspace.auditPeriod
  ...
  return (
    <>
      ...
      <Subheading className="mt-10">Checklist for {checklistPeriodLabel}</Subheading>
          ...
          {uploadChecklist.map((item) => (
            <TableRow key={item.category}>
              <TableCell>
                ...
                <div className="text-zinc-500">{periodScopeLabel(item)}</div>
              </TableCell>
              <TableCell>
...
function periodScopeLabel(item: UploadChecklistItem) {
  if (item.periodScope === 'rolled_forward') {
    return `Rolled forward from ${uploadPeriodLabel(item.latestUpload) ?? 'previous period'}`
  }

  return item.rollsForward ? 'Can roll forward' : 'Resets monthly'
}

function uploadPeriodLabel(upload: UploadChecklistItem['latestUpload']): string | undefined {
  const value = upload?.metadata.periodLabel
